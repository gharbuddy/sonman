...
export default function App() {
  ...
  return (
    <SafeLayout>
      ...
      {screen === "cart" && <Cart items={cartProducts} quantities={cart} subtotal={subtotal} error={orderError} onQuantity={setCartQuantity} onSaveLater={(id) => { if (!wishlist.includes(id)) toggleWishlist(id); void setCartQuantity(id, 0); }} onCheckout={() => setScreen("checkout")} />}
      {screen === "checkout" && <Checkout address={defaultAddress} items={cartProducts} quantities={cart} subtotal={subtotal} quote={deliveryQuote} busy={placingOrder} error={orderError} onBack={() => setScreen("cart")} onAddresses={() => setScreen("addresses")} onPlaceOrder={placeOrder} />}
...
function Home({ profile, address, products, productsError, wishlist, onWishlist, onProfile, onAddresses, onCategories, onListing, onProduct }: { profile?: UserProfile; address?: CustomerAddress; products: Product[]; productsError: string; wishlist: string[]; onWishlist: (id: string) => void; onProfile: () => void; onAddresses: () => void; onCategories: () => void; onListing: (category?: string) => void; onProduct: (product: Product) => void }) {
  ...
  return (
    <ScreenShell contentContainerStyle={styles.homeScreen}>
      ...
      <SectionHeader title="More to explore" action="View all" onPress={() => onListing("Recommended")} />
...
function Details({ product, inCart, saved, onBack, onCart, onWishlist, onBuy }: { product: Product; inCart: boolean; saved: boolean; onBack: () => void; onCart: () => void; onWishlist: () => void; onBuy: () => void }) {
  const { width } = useWindowDimensions();
  ...
  return (
    <ScreenScroll>
      <PageHeader title="Product details" onBack={onBack} action={saved ? "♥" : "♡"} onAction={onWishlist} />
...
function Cart({ items, quantities, subtotal, error, onQuantity, onSaveLater, onCheckout }: { items: Product[]; quantities: Record<string, number>; subtotal: number; error: string; onQuantity: (id: string, quantity: number) => void; onSaveLater: (id: string) => void; onCheckout: () => void }) {
  return (
    <ScreenScroll>
...
function Profile({ profile, wishlistCount, onEdit, onWishlist, onAddresses, onHelp, onNotifications, onNotificationSettings, onLogout }: { profile?: UserProfile; wishlistCount: number; onEdit: () => void; onWishlist: () => void; onAddresses: () => void; onHelp: () => void; onNotifications: () => void; onNotificationSettings: () => void; onLogout: () => void }) {
  return (
    <ScreenScroll>
      ...
      <View style={styles.profileMenuCard}><ProfileRow icon="♥" title="Wishlist" subtitle={`${wishlistCount} saved ${wishlistCount === 1 ? "item" : "items"}`} onPress={onWishlist} /><ProfileRow icon="PIN" title="Saved addresses" subtitle="Add, edit, or choose your default" onPress={onAddresses} /><ProfileRow icon="NOT" title="Notifications" subtitle="View your notification history" onPress={onNotifications} last /></View>
...
function SearchBar({ placeholder = "Search products, categories, and more", value, onChange }: { placeholder?: string; value?: string; onChange?: (value: string) => void }) {
  return <View style={styles.searchShell}><View style={styles.search}><Text style={styles.searchIcon}>⌕</Text><TextInput style={styles.searchInput} placeholder={placeholder} placeholderTextColor={palette.muted} value={value} onChangeText={onChange} /><Text style={styles.mic}>SEARCH</Text></View></View>;
}
