...
function Checkout({ address, items, quantities, subtotal, quote, busy, error, onBack, onAddresses, onPlaceOrder }: { address?: CustomerAddress; items: Product[]; quantities: Record<string, number>; subtotal: number; quote: ReturnType<typeof quoteDelivery>; busy: boolean; error: string; onBack: () => void; onAddresses: () => void; onPlaceOrder: () => void }) {
  ...
  return (
    <ScreenScroll>
      ...
      <CheckoutSection icon="PAY" title="Payment method" action="Paytm"><View style={styles.paytmPanel}><View style={styles.paytmBrand}><View style={styles.paytmMark}><Text style={styles.paytmMarkText}>tm</Text></View><View style={styles.flex}><Text style={styles.rowTitle}>Paytm secure checkout</Text><Text style={styles.smallMuted}>UPI, Paytm Wallet, cards, net banking</Text></View></View><View style={styles.paymentOptions}>{["UPI", "Paytm Wallet", "Cards", "Net banking"].map((method) => <Text key={method} style={[styles.paymentOption, paymentMethod === method && styles.paymentOptionActive]} onPress={() => setPaymentMethod(method)}>{method}</Text>)}</View><Text style={styles.paymentAssurance}>Order is created only after Paytm payment verification succeeds.</Text></View></CheckoutSection>
