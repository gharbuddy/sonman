...
import { createCouponsService, type AppliedCoupon } from "./coupons";
...
export default function App() {
  ...
  const couponsService = useMemo(() => createCouponsService(authService.supabase), []);
  ...
  const openUpiPayment = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        cu: "INR",
        tn: appliedCoupon ? `Sonman Order ${appliedCoupon.code}` : "Sonman Order",
      });
  ...
  const confirmUpiPayment = async (paymentReference?: string) => {
    ...
    try {
      ...
      }, CHECKOUT_DISTANCE_KM, paymentReference, appliedCoupon);
      setCart({});
  ...
  const payableAmount = Math.max(0, subtotal - couponDiscount) + (deliveryQuote.fee ?? 0);

  useEffect(() => {
    if (!appliedCoupon) return;
    void couponsService.validateCoupon(appliedCoupon.code, subtotal)
      .then(setAppliedCoupon)
      .catch(() => setAppliedCoupon(undefined));
  }, [appliedCoupon?.code, couponsService, subtotal]);
  ...
  return (
    <SafeLayout>
      ...
      {screen === "checkout" && <CheckoutScreen address={defaultAddress} items={cartProducts} quantities={cart} subtotal={subtotal} quote={deliveryQuote} busy={placingOrder} error={orderError} appliedCoupon={appliedCoupon} onCouponChange={setAppliedCoupon} validateCoupon={couponsService.validateCoupon} onBack={() => setScreen("cart")} onAddresses={() => setScreen("addresses")} onPlaceOrder={openUpiPayment} />}
      {screen === "payment-confirmation" && <PaymentConfirmationScreen total={payableAmount} busy={placingOrder} error={orderError} onPaid={confirmUpiPayment} onFailed={cancelUpiPayment} />}
