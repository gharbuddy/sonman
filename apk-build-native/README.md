...
`initiate` returns `{ orderId, txnToken, amount, mid, callbackUrl, isStaging }`
for the native Paytm All-in-One SDK. `verify` must confirm `TXN_SUCCESS` with Paytm, and only then call
...
Paytm native checkout requires a development build or APK. It does not work in
Expo Go.
