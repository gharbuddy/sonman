...
function Home({ profile, address, products, productsError, onProfile, onAddresses, onCategories, onListing, onProduct }: { profile?: UserProfile; address?: CustomerAddress; products: Product[]; productsError: string; onProfile: () => void; onAddresses: () => void; onCategories: () => void; onListing: (category?: string) => void; onProduct: (product: Product) => void }) {
  ...
  return (
    <ScreenShell contentContainerStyle={styles.homeScreen}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} pagingEnabled contentContainerStyle={styles.bannerRail}>
        {offers.map(([kicker, title, action, color], index) => (
          <Pressable key={title} style={[styles.bigBanner, { width: contentWidth, backgroundColor: color }]} onPress={() => onListing()}>
            ...
            {!!products[index]?.image && <Image source={{ uri: products[index].image }} style={styles.bannerProductImage} />}
          </Pressable>
        ))}
      </ScrollView>
      ...
      <View style={styles.productGrid}>{products.slice(0, 6).map((product) => <ProductCard key={product.id} product={product} width={cardWidth} onPress={() => onProduct(product)} />)}</View>
      {!products.length && <Empty title={productsError ? "Products unavailable" : "No products yet"} subtitle={productsError || "Approved products will appear here."} />}
      <SectionHeader title="More to explore" action="View all" onPress={() => onListing("Recommended")} />
      <View style={styles.productGrid}>{[...products].reverse().slice(0, 6).map((product) => <ProductCard key={`more-${product.id}`} product={product} width={cardWidth} onPress={() => onProduct(product)} />)}</View>
    </ScreenShell>
  );
}
