...
export async function loadActiveProducts(
  ...
  const { data, error } = await supabase
    .from("products")
    .select(
      "id, vendor_id, name, description, price, delivery_size, variants, categories(name), vendors!inner(approval_status), product_images(storage_path, is_primary, sort_order)",
    )
    .eq("is_active", true)
